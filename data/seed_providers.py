...
"""
Seeds providers so EVERY category has at least 2 providers.
...
FIRST_NAMES = ["John", "Mary", "Peter", "Grace", "David", "Faith", "James", "Mercy",
               ...
               "Joyce", "Felix", "Winnie", "Anthony", "Beatrice", "Collins",
               "Sheila", "Victor", "Naomi", "Isaac", "Cynthia", "Duncan", "Purity",
               "Elijah", "Ruth", "Patrick", "Vivian", "George", "Rose"]
LAST_NAMES  = ["Mwangi", "Otieno", "Kamau", "Wanjiru", "Kimani", "Achieng", "Kiptoo",
               ...
               "Njenga", "Mutiso", "Njuguna", "Ochieng", "Wekesa"]
...
def seed():
    ...
    # Wipe old seed data so we start clean
    print("🧹 Clearing old provider data...")
    db.query(Service).delete()
    db.query(ProviderProfile).delete()
    db.query(User).filter(User.role == UserRole.provider).delete()
    db.commit()
    ...
    # ---- Phase 1: guarantee ≥2 providers per category ----
    print("📦 Phase 1: ensuring every category has providers...")
    all_slugs = list(SERVICE_TEMPLATES.keys())
    phone_counter = 70000001

    for slug in all_slugs:
        cat = cats.get(slug)
        if not cat:
            continue
        for _ in range(2):  # 2 providers per category
            first = random.choice(FIRST_NAMES)
            last  = random.choice(LAST_NAMES)
            sub   = random.choice(SUB_COUNTIES)
            phone = f"2547{phone_counter:08d}"
            phone_counter += 1
            ...
            profile = ProviderProfile(
                user_id=user.id,
                bio=f"Experienced {first} offering quality {slug.replace('-', ' ')} services in {sub} and around Nairobi.",
            ...
            # Add all template services for this category
    ...
    # ---- Phase 2: add 10 multi-skilled providers for variety ----
    print("📦 Phase 2: adding multi-skilled providers...")
    for i in range(10):
        first = random.choice(FIRST_NAMES)
        last  = random.choice(LAST_NAMES)
        sub   = random.choice(SUB_COUNTIES)
        ...
        profile = ProviderProfile(
            user_id=user.id,
            bio=f"Multi-skilled fundi {first} offering various services.",
            years_experience=random.randint(3, 20),
            verification_status=VerificationStatus.verified if i % 2 == 0 else VerificationStatus.pending,
            avg_rating=round(random.uniform(4.0, 5.0), 2),
            total_reviews=random.randint(10, 150),
            total_bookings=random.randint(20, 300),
        )
        db.add(profile)
        db.flush()

        # 3 random categories each
        for slug in random.sample(all_slugs, 3):
            cat = cats.get(slug)
            if not cat:
                continue
            for title, lo, hi in SERVICE_TEMPLATES[slug][:1]:
                price = random.randint(lo, hi)
                svc = Service(
                    provider_id=profile.id,
                    category_id=cat.id,
                    title=title,
                    description=f"{title} by {first}.",
                    base_price=price,
                    price_unit="per job",
                )
                db.add(svc)
    ...
    print(f"✅ Every category now has ≥2 providers")
